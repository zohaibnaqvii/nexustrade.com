        ...
        <div className="min-h-screen bg-[#05070a] relative overflow-hidden flex flex-col hero-pattern">
            ...
            <main className="flex-1 flex flex-col items-center justify-center text-center px-6 relative z-10 pt-10 pb-20">
                <div className="animate-fade-in">
                    ...
                    <div className="mt-16 max-w-lg mx-auto p-5 border border-white/10 rounded-3xl bg-white/5 backdrop-blur-xl">
                        <p className="text-[11px] text-red-500 font-black tracking-[0.2em] uppercase mb-2">⚠️ RISK WARNING & COMPLIANCE</p>
                        <p className="text-gray-400 text-[10px] font-medium leading-relaxed">
                            Trading involves significant risk to your capital. You should not invest more than you can afford to lose. 
                            By entering the terminal, you agree to our <span className="text-blue-400 underline cursor-pointer">Privacy Policy</span> and acknowledge the market volatility.
                        </p>
                    </div>
                </div>
            </main>
...
const Header: React.FC<{ user: UserProfile; activeAccount: AccountType; setActiveAccount: (a: AccountType) => void; onDeposit: () => void; onWithdraw: () => void; onKYC: () => void; onToggleHistory: () => void; }> = ({ user, activeAccount, setActiveAccount, onDeposit, onWithdraw, onKYC, onToggleHistory }) => (
            ...
            <div className="relative group">
                ...
                <div className="absolute right-0 top-[120%] w-64 bg-[#161b22] border-2 border-white/10 rounded-2xl shadow-[0_20px_80px_rgba(0,0,0,0.9)] opacity-0 group-hover:opacity-100 pointer-events-none group-hover:pointer-events-auto transition-all transform origin-top-right p-4 z-[99999] scale-95 group-hover:scale-100">
                    ...
                    <div className="space-y-2">
                        ...
                        <a href="https://t.me/nexustradepr" target="_blank" rel="noopener noreferrer" className="w-full text-left p-3 hover:bg-blue-500/10 rounded-xl text-xs font-black flex items-center gap-3 transition-all text-blue-400 uppercase tracking-widest border border-transparent hover:border-blue-500/30">Support Center</a>
...
const MainApp = ({ user }: { user: UserProfile }) => {
    ...
    useEffect(() => {
        if (user.kycStatus === KYCStatus.PENDING && user.kycData?.submittedAt) {
            const timeout = setTimeout(async () => {
                ...
                } catch (e) { console.error("KYC Auto-approve failed:", e); }
            }, 180000);
    ...
    useEffect(() => {
        const interval = setInterval(async () => {
            ...
            for (const t of openTrades) {
                ...
                await updateDoc(doc(db, 'trades', t.id), { status: win ? 'WIN' : 'LOSS', profit: win ? t.amount * PAYOUT_PERCENTAGE : -t.amount });
                if (win) {
    ...
    const handleTrade = useCallback(async (direction: 'UP' | 'DOWN', amount: number, duration: number) => {
        ...
        if (currentBal <= 0 || amount > currentBal) { alert("Insufficient funds!"); return; }
        if (priceRef.current === 0) { alert("Syncing..."); return; }
        try {
            const newTradeData = { userId: user.uid, symbol: currentPair.name, direction, amount, entryPrice: priceRef.current, expiryTime: Date.now() + (duration * 1000), createdAt: Date.now(), accountType: activeAccount, status: 'OPEN', profit: 0 };
            const balanceField = activeAccount === AccountType.REAL ? 'realBalance' : 'demoBalance';
            await Promise.all([ updateDoc(doc(db, 'users', user.uid), { [balanceField]: increment(-amount) }), addDoc(collection(db, 'trades'), newTradeData) ]);
    ...
    const handleWithdrawSubmit = async (data: any) => {
        try {
            ...
            if (amount > user.realBalance) { alert("Insufficient balance!"); return; }
            const txData = { userId: user.uid, type: 'WITHDRAWAL' as const, amount: amount, method: data.method, address: data.details, status: TransactionStatus.PENDING, createdAt: Date.now() };
            await updateDoc(doc(db, 'users', user.uid), { realBalance: increment(-amount) });
            const docRef = await addDoc(collection(db, 'transactions'), txData);
            await sendTelegramNotification(`💸 <b>WITHDRAWAL REQUEST</b>\n\nUser: ${user.email}\nAmount: $${data.amount}\nMethod: ${data.method}`, docRef.id);
    ...
    const handleKYCSubmit = async (data: any) => {
        try {
            await updateDoc(doc(db, 'users', user.uid), { kycStatus: KYCStatus.PENDING, kycData: { fullName: data.fullName, idNumber: data.idNumber, frontImage: data.frontImage, backImage: data.backImage, submittedAt: Date.now() } });
            await sendTelegramNotification(`🆔 <b>KYC SUBMISSION</b>\nUser: ${user.email}\nName: ${data.fullName}`);
            setKYCOpen(false);
            alert("Documents received. ETA 3 mins.");
        } catch (err) { alert("KYC failed."); }
    };

    return (
        <div className="flex flex-col h-screen w-full bg-[#05070a] overflow-hidden">
            <Header user={user} activeAccount={activeAccount} setActiveAccount={setActiveAccount} onDeposit={() => setDepositOpen(true)} onWithdraw={() => setWithdrawOpen(true)} onKYC={() => setKYCOpen(true)} onToggleHistory={() => setHistoryOpen(true)} />
            <main className="flex-1 flex flex-col relative z-0 overflow-hidden">
                <div className="flex-[0.7] relative flex flex-col border-b-2 border-white/10 shadow-xl overflow-hidden">
                    ...
                    <div className="flex-1 min-h-0">
                        <TradingViewChart key={`${currentPair.symbol}-${chartResolution}-${activeAccount}`} symbol={currentPair.symbol} trades={trades.filter(t => t.accountType === activeAccount)} onPriceUpdate={p => priceRef.current = p} candleCountdown={candleCountdown} resolution={chartResolution} currentTimeframe={currentTimeframe} onResolutionChange={(res, tf) => { setChartResolution(res); setCurrentTimeframe(tf); setCandleCountdown(res); }} />
                    </div>
                </div>
