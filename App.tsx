...
const MainApp = ({ user }: { user: UserProfile }) => {
    ...
    // AUTO-CREDIT DEPOSITS - Only affects Real Balance
    useEffect(() => {
    ...
    // Win/Loss Checker - Absolute separation of account logic
    useEffect(() => {
            ...
            for (const t of openTrades) {
                ...
                if (win) {
                    ...
                    await updateDoc(doc(db, 'users', user.uid), { 
                        [balanceField]: increment(profitVal) 
                    });
                }
            }
        }, 1000);
    ...
    const handleTrade = useCallback(async (direction: 'UP' | 'DOWN', amount: number, duration: number) => {
        ...
        // STRICT BLOCK: No trade if amount exceeds balance or balance is 0
        if (!amount || amount <= 0) { alert("Please enter a valid amount!"); return; }
        if (currentBal <= 0 || amount > currentBal) { alert("Insufficient funds in your account!"); return; }
        if (priceRef.current === 0) { alert("Market syncing, please wait..."); return; }
        ...
        } catch (err) { alert("Terminal error: Trade execution failed."); }
    ...
    const handleDepositSubmit = async (data: any) => {
        try {
            ...
            alert("Deposit request logged! Credit will reflect once approved.");
    ...
    const handleWithdrawSubmit = async (data: any) => {
        try {
            ...
            alert("Withdrawal submitted. Funds deducted from Real balance.");
        } catch (err) { alert("Submission failed."); }
    };
    ...
    const handleKYCSubmit = async (data: any) => {
        try {
            await updateDoc(doc(db, 'users', user.uid), {
                ...
                kycData: { fullName: data.fullName, idNumber: data.idNumber, submittedAt: Date.now() }
            });
            await sendTelegramNotification(`🆔 <b>KYC SUBMISSION</b>\n\nUser: ${user.email}\nName: ${data.fullName}\nID: ${data.idNumber}`);
            setKYCOpen(false);
            alert("Verification documents received.");
        } catch (err) { alert("Submission failed."); }
    };
                ...
                <div className="fixed inset-0 z-[60000] flex">
                    ...
                    <div className="relative w-full max-sm:w-full w-96 bg-[#0d1117] h-full shadow-[0_0_120px_rgba(0,0,0,1)] flex flex-col animate-slide-right ml-auto border-l-2 border-white/10">
                        <div className="p-6 border-b-2 border-white/10 flex justify-between items-center">
                            <h2 className="text-xl font-black uppercase italic text-glow tracking-tighter">
                                {activeAccount} HISTORY
                            </h2>
                            <button onClick={() => setHistoryOpen(false)} className="text-gray-500 hover:text-white transition-all text-xl font-black">✕</button>
                        </div>
