...
export const KYCModal: React.FC<{ isOpen: boolean; onClose: () => void; onSubmit: (data: any) => void }> = ({ isOpen, onClose, onSubmit }) => {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Identity Verification (KYC)">
            <div className="space-y-4">
                <div className="bg-blue-500/10 border border-blue-500/20 p-3 rounded-lg text-[10px] text-blue-400 font-black uppercase tracking-[0.2em]">
                    Institutional Security: Verification required for withdrawal. ETA: 3 Minutes.
                </div>
                <input 
                    type="text" placeholder="Full Legal Name" className="w-full bg-[#0d1117] border border-[#30363d] rounded-lg p-3 text-white outline-none focus:border-blue-500 font-bold"
                    value={fullName} onChange={(e) => setFullName(e.target.value)}
                />
                <input 
                    type="text" placeholder="ID / Passport / CNIC Number" className="w-full bg-[#0d1117] border border-[#30363d] rounded-lg p-3 text-white outline-none focus:border-blue-500 font-bold"
                ...
                <div className="grid grid-cols-2 gap-4">
                    <label className={`relative border-2 border-dashed ${frontImg ? 'border-green-500 bg-green-500/5' : 'border-[#30363d]'} p-4 text-center rounded-lg cursor-pointer hover:border-blue-500 transition-colors`}>
                        <span className="text-[10px] text-gray-500 font-black uppercase tracking-widest">{frontImg ? 'FRONT DONE' : 'FRONT ID PHOTO'}</span>
                    ...
                    <label className={`relative border-2 border-dashed ${backImg ? 'border-green-500 bg-green-500/5' : 'border-[#30363d]'} p-4 text-center rounded-lg cursor-pointer hover:border-blue-500 transition-colors`}>
                        <span className="text-[10px] text-gray-500 font-black uppercase tracking-widest">{backImg ? 'BACK DONE' : 'BACK ID PHOTO'}</span>
                ...
                <button 
                    ...
                    className="w-full py-4 bg-blue-600 hover:bg-blue-700 rounded-lg font-black text-white text-[10px] uppercase tracking-[0.3em] mt-4 disabled:opacity-30 transition-all shadow-xl"
                >
