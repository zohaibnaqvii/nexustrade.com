...
export const KYCModal: React.FC<{ isOpen: boolean; onClose: () => void; onSubmit: (data: any) => void }> = ({ isOpen, onClose, onSubmit }) => {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Identity Verification (KYC)">
            <div className="space-y-4">
                <input 
                    type="text" placeholder="Full Name" className="w-full bg-[#0d1117] border border-[#30363d] rounded-lg p-3 text-white outline-none"
                    value={fullName} onChange={(e) => setFullName(e.target.value)}
                />
                <input 
                    type="text" placeholder="ID / Passport Number" className="w-full bg-[#0d1117] border border-[#30363d] rounded-lg p-3 text-white outline-none"
                    value={idNumber} onChange={(e) => setIdNumber(e.target.value)}
                />
                <div className="grid grid-cols-2 gap-4">
                    <div className="border-2 border-dashed border-[#30363d] p-4 text-center rounded-lg cursor-pointer hover:border-blue-500 transition-colors">
                        <span className="text-xs text-gray-500">Front ID Photo</span>
                        <input type="file" className="hidden" />
                    </div>
                    <div className="border-2 border-dashed border-[#30363d] p-4 text-center rounded-lg cursor-pointer hover:border-blue-500 transition-colors">
                        <span className="text-xs text-gray-500">Back ID Photo</span>
                        <input type="file" className="hidden" />
                    </div>
                </div>
                <button 
                    disabled={!fullName || !idNumber}
                    onClick={() => onSubmit({ fullName, idNumber })}
                    className="w-full py-3 bg-blue-600 hover:bg-blue-700 rounded-lg font-bold mt-4"
                >
                    Submit for Approval
                </button>
            </div>
